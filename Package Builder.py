...
def check_unique(list_1,list_2=packages):
    checklist= list_1
    check = True
    return_list=[]
        ...
        for j in range(len(checklist)):
            for k in range(len(checklist)):
                for l in range(len(checklist)):
                    if i!=j!=k!=l and j!=l!=i!=k:
                        return_list.append([checklist[i],checklist[j],checklist[k],checklist[l]])
...
#set up GUI
gui.theme('DarkAmber')
layout=[[gui.T("")],[gui.Text('choose a list of items: '),gui.Text(key='_file1_'),gui.FileBrowse(key='file1')],[gui.T('')],[gui.Text('choose a template: '),gui.Text(key="_file2_"),gui.FileBrowse(key="file2")],[gui.Text('packages built: '),gui.Text(key = 'pckg')],[gui.Button('Submit')]]
...
while True:
    ...
    #run builder by clicking submit
        ...
        for item in base_item_list:
            for item2 in items:
                if item.cat1 == 'WALL OVEN' and item2.cat1=='RANGE HOOD':continue
                pack = ['','','','','','','']
        ...
        for pack in packages:
            ...
            if m==5:
                if item.cat1 == 'RANGE HOOD' and 'COOKTOP' not in pack[0].cat2:continue
